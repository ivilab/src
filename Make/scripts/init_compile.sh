...
#
# Description:
#    This script provides the IVI build environment variables to bash users. It
...
#    bash). 
#
#    This script assumes that IVI_SRC_PATH points to a directory which has Make
...
#    default location of this script is ${IVI_SRC_PATH}/Make/scripts, it seems
#    safe to assume that IVI_SRC_PATH can be set as it is implicit in calling
#    this script anyway. 
#
###############################################################################
...
# works. 
echo source ${IVI_SRC_PATH}/Make/init_compile | tcsh -s 

if [[ $? == 0 ]] 
then
    source ${TMPDIR}/${USER}/ivi_env.sh 
    if [[ $? != 0 ]] 
    then
        echo "Sourcing ${TMPDIR}/${USER}/ivi_env.sh failed."
        return 1
    fi 
else 
    echo "Sourcing  ${IVI_SRC_PATH}/Make/init_compile within tcsh failed."
    return 1
fi 
