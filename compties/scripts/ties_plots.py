...
def _draw_basic_plot(ax, title, onam, tn, tm, o0, o1, m0, m1, prediction_fraction):
    '''
    ...
    '''
    handles = ax.plot(tn, o0, 'r', tn, o1, 'b', tm, m0, 'r--', tm, m1, 'b--')
    ax.set_title(title)
...
def _plot_without_legend(title, d, onam, tn, tm, obs0, obs1, model0, model1):
    'Generate and return a figure, on which we draw the basic plot.'
    fig = matplotlib.figure.Figure()
    ax = fig.add_subplot(111)
    _draw_basic_plot(ax, title, onam, tn, tm, obs0, obs1, model0, model1,
        prediction_fraction=0.2)
    return fig


def _plot_with_legend(title, distinguisher, onam, tn, tm, obs0, obs1, model0, model1):
    '''
    ...
    h = _draw_basic_plot(ax, title, onam, tn, tm, obs0, obs1,
...
def _gen_a_model_plot(plot_path_stem, onam, dnam, dyad_num, model_name, data, model):
    ...
    tm = sorted(model.keys()) # time as array of native Python ints
    ...
    model0 = numpy.ma.array([nonemask(model[t][0]) for t in tm])
    model1 = numpy.ma.array([nonemask(model[t][1]) for t in tm])
    ...
    fig = plot_fn(title, dnam, onam, numpy.array(ti), numpy.array(tm), 
                  obs0, obs1, model0, model1)
...
def _get_fname(dyadnum, num_obs, obs_cat_name, modelname, plotdir, cname=None):
    '''
...
def _gen_model_plts(modelname, dnam, plotdir, data, modelfns, cname=None):
    '''
    ...
    for m in modelfns:
        ...
        for oc in data.iterkeys():
            ...
            if(cname == None):
                ...
                group_path = os.path.dirname(os.path.realpath(m))
                group_index_fp = os.path.join(group_path, 'group_index.txt');
                with open(group_index_fp) as f:
                        group_index = f.readlines()
                group_index = [x.strip() for x in group_index] 
                group_plotdir = os.path.join(plotdir, cname+'-' + group_index[0]);
                ties_log.failsafe_mkdir(group_plotdir)
                fname = _get_fname(dyadnum, len(cio), oc, modelname, group_plotdir)
...
def generate_individual_plots(args):
    ...
    * data_dir
    ...
    ties_log.log_debug('gen_individual_plots() entry, bd=' #+ str(baseline_dir)
    ...
    if args.categorical_mod is not None:
        dflist = glob.glob(os.path.join(data_dir, args.categorical_mod + '-*',
                          glob4d + '.txt'))
    if args.single_fold is None:
        if args.categorical_mod is not None:
            fold_states = os.path.join('fold-[0-9][0-9]', 
                          'testing', args.categorical_mod + '-*', 
                          glob4d, 'all_states', 
                          #glob4d, 
                          'obs_states.txt')
        else:
        ...
        if args.categorical_mod is not None:
            fold_states = os.path.join(args.categorical_mod + '-*',
                          glob4d, 'all_states', 'obs_states.txt')
    ...
    if args.single_fold is not None:
        ...
        _gen_model_plts(args.model_name, args.distinguisher,
           args.output_dir_plots, dyad_obs, 
           sorted(mslist, None, kfold_sorter), args.categorical_mod)

if __name__ == '__main__':
    log_pid = ties_log.start_log_with_args()
    try:
        args = ties_options.parse_my_args()
