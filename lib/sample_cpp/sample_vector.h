/* $Id: sample_vector.h 25499 2020-06-14 13:26:04Z kobus $ */
/* =========================================================================== *
   |
...
private:
    ...
    Independent_gaussian_proposer(const ivi::Vector& covariance) : 
    ...
    Mh_proposal_result operator()(const SimpleVector& m, SimpleVector& mp) const
    {
        ...
        {
            mp[i] = m[i] + ivi::sample(ivi::STD_NORMAL) * scale_[i];
        }
...
private:
    ...
    ivi::Matrix chol_cov_;
    double log_lambda_;
    double sqrt_t_;

public:
    /** @brief  Construct a Mv_gaussian_proposer. */
    Mv_gaussian_proposer(const ivi::Matrix& covariance) :
        chol_cov_(ivi::cholesky_decomposition(covariance)),
        log_lambda_(0.0),
        sqrt_t_(1.0)
    {}
     ...
     */
    const ivi::Matrix& get_unscaled_cholesky_covariance() const
    {
        return chol_cov_;
    }

    ivi::Matrix get_covariance() const
    {
        ivi::Matrix result = sqrt_t_ * exp(log_lambda_/2.0) * chol_cov_;
    ...
    {
#ifdef TEST
//        ivi::Matrix ref = chol_cov_;
//        ivi::Matrix test = chol_cov_;
#endif 
...
//        assert(diff < FLT_EPSILON);
     ...
     */
    void rank_1_update(ivi::Matrix& C, const VectorModel& delta, double gamma)
    {
        ...
        C += tmp;
#else
        IVI_THROW_2(ivi::Missing_dependency, "lapack");
#endif
    }
     ...
     */
    void rank_1_update_2(ivi::Matrix& C, const VectorModel& delta, double gamma)
    {
        // TODO: DEBUG ME
     ...
     */
    void rank_1_update_ref(ivi::Matrix& C, const VectorModel& delta, double gamma)
    {
        ivi::Vector tmp(delta.begin(), delta.end());
        C = C * C.transpose();
        C += gamma * (ivi::outer_product(tmp, tmp)-C);
#ifdef TEST
        ivi::Matrix m_tmp = C;
#endif

        C = ivi::cholesky_decomposition(C);

#ifdef TEST
        assert(ivi::max_abs_difference(C * C.transpose(), m_tmp) < FLT_EPSILON);
#endif
    }

private:
    // utility
    static double gauss_sample_() { return ivi::sample(ivi::STD_NORMAL); }
    ...
    void lower_triangular_product_(
            const ivi::Matrix& op1,
            const ivi::Matrix& op2,
            ivi::Matrix& out)
    {
        const size_t N = op1.get_num_cols();
        ivi::Matrix tmp(N,N, 0.0);
...
public:
     ...
     */
    inline void set_covariance(const ivi::Matrix& covariance)
    {
        Proposer& proposer = Base::get_proposer_();
        proposer = Mv_gaussian_proposer<VectorModel>(covariance);
    }

    inline void set_covariance(const ivi::Vector& covariance)
    {
        Proposer& proposer = Base::get_proposer_();
        proposer = Mv_gaussian_proposer<VectorModel>(covariance);
    }
