...
/* $Id: concat_matrices_horizontally.c 25499 2020-06-14 13:26:04Z kobus $ 
 * 
 * Author: Shreeya Jain
...
int main(void)
{
	...
	if (diff != 0.0)
	{
		p_stderr("Test failed. Program does not horizontally concatenate the matrices correctly!\n");
        return EXIT_BUG;
    }
    else
    	printf("Program horizontally concatenates the matrices correctly!\n");
...
void set_file_path(char *matrix_file, const char *filename)
{
  char base_path[] = "test_input/concat_matrices_horizontally/00/";
