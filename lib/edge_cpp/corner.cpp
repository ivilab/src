...
void Corner::read(std::istream& in)
{
    ...
    {
        IVI_THROW_2(Illegal_argument, "Missing Corner Number of Segments");
    }
    ist.str(field_value);
    ist >> num_segments;
    if (ist.fail())
    {
        IVI_THROW_2(Illegal_argument, "Invalid Corner number of segments");
    }
    ist.clear(std::ios_base::goodbit);
...
void Corner::draw( ivi::Image & img, double ir, double ig, double ib, double width)  const
{
    for(unsigned int i = 0; i < segments.size(); i++)
    {
...
void Corner::randomly_color(ivi::Image & img, double width)  const
{
    for(unsigned int i = 0; i < segments.size(); i++)
    {
