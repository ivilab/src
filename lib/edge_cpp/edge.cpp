/* $Id: edge.cpp 25499 2020-06-14 13:26:04Z kobus $ */
/* =========================================================================== *
   |
 ...
 */
bool ivi::Edge_set::is_edge_set_consistenct()
{
    ...
    for(unsigned int i = 0; i < m_edge_set->num_edges; i++)
    {
         ...
         ivi_c::Edge c_edge = m_edge_set->edges[i];
         ...
         for(unsigned int j = 0; j < c_edge.num_points; j++)
         {
               ...
               ivi_c::Edge_point c_ept = c_edge.points[j];
...
Edge_set_ptr edge_image_to_edge_points(const Image& i, bool oriented)
{
    ...
    std::vector<ivi_c::Edge_point> edges;
    edges.reserve(512);
    ...
    for(int row = 0; row < i.get_num_rows(); row++)
    {
        ...
        {
            const ivi_c::Pixel& px =  i(row, col);
