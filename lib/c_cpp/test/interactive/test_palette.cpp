 ...
 */
/*
 * $Id: test_palette.cpp 25499 2020-06-14 13:26:04Z kobus $
 *
 * Tab size:  4
 */
...
void sort( ivi::Palette& p )
{
    ...
    for( size_t working_len = p.size(); 1 < working_len; --working_len )
    {
        ...
        ivi::Vector dist2(int(working_len), DBL_MAX);
        ivi::Int_vector winner(working_len, int(p.size()));
        //winner.assign( working_len, p.size() );
        for( size_t kix = 0; kix < working_len; ++kix )
        {
            vk = ivi::hsluma_space( p[ kix ] );
            for( size_t jix = 0; jix < working_len; ++jix )
            {
                if ( kix == jix ) continue;
                double d2 =
                    ( vk - ivi::hsluma_space( p[ jix ] ) ).magnitude_squared();
                if ( d2 < dist2[ kix ] )
                {
                    dist2[ kix ] = d2;
                    winner[ kix ] = jix;
            ...
            IVI(ASSERT( winner[ kix ] < int(p.size()) ));
        }
        ...
        IVI(ASSERT( int(mcix) != winner[ mcix ] ));
        ...
        else
        {
            ...
            IVI(ASSERT( mxix != mnix ));
            IVI(ASSERT( mxix <= working_len - 1 ));
...
int main()
{
    IVI( EPETE( ivi_signal( SIGCHLD, SIG_IGN ) ) );
    const int BLOCK = 50, NUMBLOCK = 10;
    ivi::Image im( BLOCK * NUMBLOCK, BLOCK * NUMBLOCK );
    ivi::Palette pal( 100 );
