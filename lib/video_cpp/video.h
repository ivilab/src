/* $Id: video.h 25499 2020-06-14 13:26:04Z kobus $ */
/* {{{=========================================================================== *
   |
...
#ifndef IVI_CPP_I_VIDEO_H
#define IVI_CPP_I_VIDEO_H
...
public:
    ...
    {
#if defined(IVI_HAVE_FFMPEG) && AVFORMAT_IS_RECENT
        ...
        if(avformat_open_input(&pFormatCtx, fname.c_str(), NULL, NULL)!=0)
            IVI_THROW_3(ivi::IO_error, "Couldn't open file %s", (fname.c_str()));

        /* Retrieve stream information */
        if(avformat_find_stream_info(pFormatCtx, NULL)<0)
            IVI_THROW_2(ivi::Runtime_error, "Failed to find stream info");
    ...
    //    dump_format(pFormatCtx, 0, argv[1], false);
        ...
        if(videoStream==-1)
            IVI_THROW_2(ivi::Runtime_error, "Didn't find a video stream");
        ...
        if(pCodec==NULL)
            IVI_THROW_2(ivi::Runtime_error, "Codec not found");
        ...
        if(pFrameRGB==NULL)
            IVI_THROW_2(ivi::Runtime_error, "Failed to allocate AVFrame");
...
#else
        IVI_THROW_2(Missing_dependency, "ffmpeg");
#endif
    }
    ...
    void load_images(Iterator first, Iterator last, float frame_rate = 30.0)
    {
        ...
        for(; first != last; first++, i++)
        {
            ...
            else
            {
                ...
                {
                    IVI_THROW_2(Illegal_argument, "All images must be same size.");
                }
            }
