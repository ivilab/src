/* $Id: psi_likelihood.cpp 25499 2020-06-14 13:26:04Z kobus $ */
/* {{{=========================================================================== *
   |
...
{
    if(n < 0) IVI_THROW_2(ivi::Illegal_argument, "n must be a non-negative integer.");
    if(n == 0 || n == 1) return 0;
...
//    ASSERT(num_capsule_frames == num_frames); // this will fail
    ...
    video_likelihood += log(ivi::pdf(frame_count_penalty_, fabs((double) num_capsule_frames - (double) num_frames)));
...
std::vector<std::pair<double, Bbox> > Cartwheel_likelihood::blocks_to_boxes(const std::vector<CartWheel::BoxStatePtr>& blocks) const
{
    ...
    for(size_t i = 0; i < blocks.size(); i++)
    {
        ...
        // get depth
        double depth = get_depth(to_ivi(blocks[i]->getPosition()), cam_);
...
double Frame_likelihood::compute_visibility(const std::vector<Bbox>& all_boxes, size_t box_i) const
{
    ...
    std::vector<ivi::Vector> cell_centers;
    ...
    for(size_t y_i = 0; y_i < NUM_SUBDIVISIONS; y_i++)
    {
        ...
        const ivi::Vector cell_center(x,y);

        if(frame_bounds_.contains(cell_center))
        {
            cell_centers.push_back(ivi::Vector(x, y));
 ...
 */
ivi::Matrix Frame_likelihood::get_affinity(
        ...
        ) const
{
    ...
    ivi::Matrix affinities(data.size(), model.size()+1);
...
std::vector<size_t> Frame_likelihood::get_correspondence_greedily( const ivi::Matrix& affinities) const
{
    using namespace std;
...
//    std::vector<std::vector<Cuboid> > blocks_frames = simulator_.get_box_states();
    ...
    // (b) re-parameterize the model to ensure constant frame length
    //     (i.e. sample over transition time)
    video_likelihood += log(ivi::pdf(frame_count_penalty_, fabs((double) num_entity_frames - (double) num_frames_)));
