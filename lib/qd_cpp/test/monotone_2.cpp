 ...
 */
/*
 * $Id: monotone_2.cpp 25499 2020-06-14 13:26:04Z kobus $
 */
...
Doubly_connected_edge_list complicated()
{
    ...
    d = d.merge(ivi::qd::ctor_from_edge_list(obstacles));
    ...
    if (VISUALIZATION > 0)
    {
        ...
        if (0)
        {
            RatPoint::Rat lcd = ivi::qd::common_denominator(d),
...
int test1()
{
    ...
    if (VISUALIZATION > 0)
    {
        ...
        for (size_t i = 1; VISUALIZATION > 1 && i < d.get_face_table().size(); ++i)
        {
            ...
            ff << xml_output(exp);
            std::ofstream fg((std::string(buf)+"svg").c_str());
            fg << ivi::qd::draw_dcel_as_svg(exp);
        }
    }

    const Doubly_connected_edge_list e = ivi::qd::make_faces_ymonotone(d);
...
int test2()
{
    ...
    const Doubly_connected_edge_list
        e = ivi::qd::make_faces_ymonotone(t5);
...
int main(int argc, char** argv)
{
    ...
    try
    {
        if (time_factor > 1) IVI(EPETE(test1()));
        IVI(EPETE(test2()));
    }
